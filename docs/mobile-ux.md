...
Baseline updates require intentional UI review. Run `UPDATE_VISUAL_BASELINES=1 rtk node --test --test-concurrency=1 electron/tests/web-server-visual.test.mjs`, inspect every changed PNG under the matching OS directory in `electron/tests/visual-baselines/h5`, then rerun without the environment variable. macOS and Linux use separate reviewed baselines because Chromium's OS graphics stack changes glyph rasterization and layout; the test automatically selects `darwin` or `linux`, and does not fall back across platforms. Do not accept a baseline solely because a test produced it. The H5 harness uses the repository-pinned Noto Sans SC subset under `electron/renderer/assets/fonts` only in visual-test mode, waits for it to load, and permits at most 0.2% of pixels to differ after ignoring per-channel raster noise of 12. Dimensions must match exactly, so layout and visible styling regressions still fail. On mismatch, CI uploads `electron/tests/visual-artifacts/**`, including actual and diff PNGs, for review.
