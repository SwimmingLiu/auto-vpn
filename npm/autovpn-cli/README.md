...
- `AUTOVPN_NO_PYTHON=1` disables implicit Python backend resolution and default Python runtime stage fallback. Use it as a v3 readiness gate. Empty offline runs now complete in Node, and Node has direct HTTP speedtest and availability runtimes. Node also has the Mihomo runtime lifecycle and opt-in controller delay probe path via `AUTOVPN_SPEEDTEST_RUNTIME=mihomo`; wiring download speed tests and availability checks through the local Mihomo proxy remains follow-up work.
