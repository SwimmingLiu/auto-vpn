...
- `AUTOVPN_NO_PYTHON=1` disables implicit Python backend resolution and default Python runtime stage fallback. Use it as a v3 readiness gate. Empty offline runs now complete in Node, and Node has direct HTTP speedtest and availability runtimes. Node also has the Mihomo runtime lifecycle and controller delay foundation; wiring full per-node Mihomo proxy speed tests and availability checks into the pipeline remains follow-up work.
