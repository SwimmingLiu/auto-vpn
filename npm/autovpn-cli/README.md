...
- `AUTOVPN_NO_PYTHON=1` disables implicit Python backend resolution and default Python runtime stage fallback. Use it as a v3 readiness gate. Empty offline runs now complete in Node, and Node has a direct HTTP speedtest runtime; full per-node Mihomo proxy speed tests and non-empty availability runtime checks remain follow-up work.
