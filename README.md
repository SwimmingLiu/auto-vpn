...
Use `AUTOVPN_NO_PYTHON=1` when validating v3 cutover readiness. It prevents the wrapper and Node orchestrator from silently resolving, installing, or launching the Python backend. Empty offline runs now complete in Node; runs that reach an unmigrated runtime boundary should fail clearly instead of falling back.
